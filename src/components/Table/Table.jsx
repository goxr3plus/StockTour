...
import translate from '../../lib/general/helper'
...
export default class Table extends Component {
  ...
  render = () => {
    ...
    let tableHeaderArray = columns.map((column) => {
      ...
      return (
        <th key={column.id} hidden={column.hidden} style={{ maxWidth: `${column.tableHeaderWidth}`, ...column.tableHeaderStyle }}>
          {component}
        </th>
      )
    })
    ...
    let rows = data.map((record, index) => {
      ...
      const row = expandable ? (
        <ExpandableRow onExpandChange={this.onExpandChange} open={expandedRows[index]} index={index} key={index} id={index}>
          {rowContent}
        </ExpandableRow>
      ) : (
